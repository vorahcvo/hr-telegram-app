  ...
  const initializeUser = async () => {
    ...
    try {
      ...
      if (testError) {
        ...
        // Не устанавливаем ошибку, чтобы не блокировать приложение
        logger.warning('⚠️ Продолжаем без подключения к базе данных');
      } else {
      ...
      if (fetchError && fetchError.code !== 'PGRST116') {
        ...
        // Не устанавливаем ошибку, чтобы не блокировать приложение
        logger.warning('⚠️ Продолжаем без данных пользователя');
        ...
        if (createError) {
          ...
          // Не устанавливаем ошибку, чтобы не блокировать приложение
          logger.warning('⚠️ Продолжаем без создания пользователя');
        } else {
      ...
      // Не устанавливаем ошибку, чтобы не блокировать приложение
      logger.warning('⚠️ Продолжаем работу приложения несмотря на ошибку');
    } finally {
